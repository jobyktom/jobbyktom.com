		...
		return this.each(function(options) {
			...
			$("#contactForm").validate({
				...
				submitHandler: function() {
					$('.holder').hide();
					$('#loading').show();
					$.post('php/mail.php',{subject:defaults.subject, name:$('#name').val(), email:$('#email').val(), comment:$('#comment').val()},
					function(data){
						...
						if( data == 'success') {
							...
							if(defaults.hideOnSubmit == true) {
								...
								$('#contactForm').animate({dummy:1}, 2000,'easeOutQuad').animate({"marginLeft": "-=450px"}, "slow");
								$('div#contactable').animate({dummy:1}, 2000,'easeOutQuad').animate({"marginLeft": "-=447px"}, "slow").animate({"marginLeft": "+=5px"}, "fast");
								$('#overlay').css({display: 'none'});	
							}
						} else {
